...
def increase_mastery_scroll(user_uuid: str, scroll_id: str, amount: int = 1):
  ...
  if scroll_id not in pgd:
    raise HTTPException(status_code=500, detail=f"User {user_uuid} premium_game_data is missing key '{scroll_id}'. Click the refresh (↻) button in the top bar to migrate your account, then try again.")
